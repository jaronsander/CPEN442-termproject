...
ngrok_address = "https://68ef-176-100-43-178.ngrok.io"+'/'
    ...
    def generateTwiml(self,ngrokPrefix, twilioToken, requestTimeout,clientName):
        ...
        twiml = "<Response>" \
                    "<Gather action=\""+ngrokPrefix + "twilio_answer/" +twilioToken+"\" method=\"POST\" input=\"dtmf\" numDigits=\"2\" timeout=\""+str(resulting_to)+"\">" \
                        "<Say>Login request for "+clientName+"'s services.\nPlease input the 2 digits that have appeared on your screen in the next "+ str(resulting_to-2) +" seconds. </Say>"\
                    "</Gather>" \
                "</Response>"
        return twiml
...
class PassgateAPI:
    ...
    def setCode(self,clientToken,p,t):
        ...
        deschedule_date = datetime.datetime.now(tz=pytz.timezone("US/Pacific")) + datetime.timedelta(seconds=actualTimeout)
        #try:
    ...
    def verifySMS(self,user_token_SMS,user_code):
        ...
        actual_code = self.SMSuserTokensMap[user_token_SMS]
